...
public class VentaDao {
    ...
    public List Listarventas(){
       ...
       String sql = "SELECT c.id AS dni, c.nombre, v.* FROM clientes c INNER JOIN ventas v ON c.id = v.cliente";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           rs = ps.executeQuery();
    ...
    public void pdfV(int idventa, int Cliente, double total, String usuario) {
        try {
            ...
            tabla.addCell(c3);
            tabla.addCell(c4);
            String product = "SELECT d.id, d.cod_pro,d.id_venta, d.precio, d.cantidad, p.id, p.nombre FROM detalle d INNER JOIN productos p ON d.cod_pro = p.id WHERE d.id = ?";
            try {
